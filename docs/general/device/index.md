...
所谓设备绑定激活，是指设备完成了与个人或团队的绑定关系，同时也对接上相应的云端服务。得力云平台在联网配置、认证接入和绑定激活都有相应的标准流程，具体说明如下：
...
设备连接网络是接入得力云平台的先决条件，不同配置的设备联网方式不尽相同。主要分为如下几种:
    ...
    1.   设备自配置联网，无需手机App协助；
    2.   设备通过蓝牙与手机App通信完成联网设置；
    3.   设备通过Wifi与手机App通信完成联网设置；

对于情况#1，由于平台无需参与网络配置，可以由设备厂商自行实现联网配置方式，但要求尽量简单。

对于情况#2，设备厂商需要将设备蓝牙名称设置为DELI_{产品型号}，方便得力App扫描定位设备。其中产品型号由平台和厂商共同定义，并写入设备固件和云平台，具体通信协议请参考《得力云设备蓝牙网络配置协议》。

对于情况#3，设备和App将采用类似微信的AirKiss通信协议方式进行连接，具体通信协议请参考《得力云设备Wifi网络配置协议》。
...
出于安全考虑，所有的智能设备都需要经过基本的认证方能接入云平台。云平台采用了MQTT协议的标准认证方式，即根据设备的唯一ID、设备账户以及密码三个信息来判断设备是否可以接入。
...
不论采用哪种认证方式，云平台要求设备ID必须全局唯一（两台相同ID的设备不能同时接入平台），由设备厂商自行生成，建议命名格式：**{产品型号}_{厂商设备唯一编码}**。例如考勤机型号DL3785W，那么某一台设备唯一编号可能是DL3785W_1234567890。
...
接入认证通过后，应同时进行时间同步和设备信息注册操作，后续第三章会讲到具体操作指令。


- ### 设备绑定激活

通常来讲，所有接入得力云的智能设备都应完成设备绑定激活，才能投入使用。绑定激活操作由得力App操作完成，设备端不需要直接参与，但设备应通过某种方式告知用户APP设备的ID。
...
设备接入平台如果成功，设备可以主动通过接口查询设备绑定状态，如果已绑定，可以将group信息填充到二维码中，设备本地不建议存储任何绑定信息，以免造成数据不一致。

生成二维码示例(一台已绑定团队ID为1000的设备):

** `http://qr.delicloud.com?from=device&action=activate&product={产品型号}&device={设备ID}&status=2&group=1000` **

**(2)对于通过与App进行蓝牙或者Wifi通信的设备: **

由于其二维码只存储静态信息，设备ID以及状态信息可通过相应的通信协议交互完成。具体数据交互协议包括在《得力云设备蓝牙网络配置协议》和《得力云设备Wifi网络配置协议》。


- ### 设备主题订阅

设备在绑定成功后，需要订阅设备产品相关的主题消息（Topic），以便平台向设备发送消息。设备在接入平台后，平台会自动会为该设备订阅一个唯一的主题，用于点对点的设备通信。除此之外，设备还可以订阅以下几个全局主题:

|主题|描述|
|----|----|
|{产品类型}|一种业务类型产品的全局消息，例如所有考勤机，定义一个产品类型KQ。|
|{产品类型}/{产品型号}|一种业务类型产品下某个具体产品型号的全局消息，例如考勤机的某个产品型号DL3785W|


除以上主题外，其他主题平台设备网关将拒绝设备订阅。

产品类型与产品型号类似，都是由云平台生成并提供给设备生产商，生产商直接固化该信息到设备程序或配置中即可。

设备订阅了以上两个主题，则平台可以针对某种产品类型或者产品类型下的某个具体产品型号发送全局广播消息，以应对某些特殊场景的需求，例如升级。
...
除了设备业务相关的数据通信接口之外，设备还有一些通用的与业务不相关的平台接口需求，例如设备接入、设备信息查询、设备开关、解绑、应用消息推送等等。平台为此定义了一系列的接口，特说明如下。


- ### 消息格式

为了便于开发和管理，平台为所有开放接口约定了统一的数据交换格式。首先，按照MQTT通信协议的设计，所有的消息都应有消息主题（Topic name），这里我们约定所有得力云平台设备接口的主题都是device。附MQTT数据包格式如下：

![](img/image003.png)

平台自定义数据格式都通过MQTT数据包的Payload消息体部分来设计。所有发送和接收到的Payload消息体都是以JSON数据格式存储的字符串，编码格式统一为UTF-8。数据格式如下:
...
对于一次完整的消息请求和响应的流程，默认情况下`mid`应相同，请求的`from`和`to`应和响应的`to`和`from`一一对应。另外，data字段中响应部分应包含响应结果成功与否。
...
以下指令是由设备端主动向平台发起的指令集，设备通过该指令集请求平台数据或通知平台设备状态数据，该指令集所有指令以1开头。
针对平台响应，data字段统一格式如下:
...
其中`code`表示响应错误码（错误码定义见5.1），如果响应失败，`msg`将包含错误的具体原因信息。



+ #### <span id="100">时间同步100</span>

设备接入成功后需要进行时间同步，以保证本地时间的准确性。生产商可选择得力云的NTP服务(例如time.delicloud.com)或公共的NTP服务皆可。当然，也可以直接通过该指令进行同步。
...
设备可直接使用响应中的time作为本地时间，精确到秒。

+ #### <span id="101">设备信息更新101</span>

设备接入成功后，可以通过该指令更新设备在云平台的一些基本信息。如果设备无信息更新，可不调用。
请求消息示例如下:
...
```
请求参数`product_version`代表当前设备固件版本。响应消息数据格式:
...
|----|----|
|status|0锁定、1正常|
|group_id|设备已绑定团队ID，如果未绑定，则为空|
|product_version|平台产品固件最新版本|
|last_access|上次接入时间（精确到秒), 初次接入为0|


如果`group_id`信息为空或者与本地缓存的`group_id`信息不一致，可能是平台已解绑该设备，此时设备应及时清理所有本地业务数据，恢复出厂数据状态。

如果平台固件版本与本地版本不一致，设备应主动查询平台最新固件版本信息并更新固件程序。


+ #### <span id="103">固件版本查询103</span>

通过该指令，设备可以查询平台目前该设备所属产品型号的最新固件版本信息，以及固件文件下载地址等。
请求消息示例如下:
...
设备根据本地版本比较决定是否需要升级，如果需要，则通过`file_path`给定的路径下载文件升级即可。另外，为了降低集中并发压力过大，**一个产品型号所有设备不要同一时刻启动升级操作**，建议在某个时间段进行随机生成一个升级启动时间，例如凌晨0点-2点，按照0-7200秒随机。

另外，平台也可以主动向设备广播固件新版本变更通知, 参考平台“**固件升级通知**”指令。

注意，由于下载升级文件时采用的HTTP协议，为了避免网络不好无法下载，应采用[HTTP通用的断点续传下载方案](https://www.w3.org/Protocols/rfc2616/rfc2616-sec3.html#sec3.12)。



+ #### <span id="104">设备主动解绑104</span>

对于安全性要求不高的设备类型，平台通过预授权允许其通过设备端主动完成解绑操作，解绑之后本地业务数据应清理干净。通过该指令设备可主动通知平台解除设备与用户团队的绑定关系。
请求消息示例如下:
```json
{
    "mid": "123456", 
...
通过该指令设备可向平台主动进行设备系统级别的（非业务级别的）异常报警。平台会将设备报警消息推送给设备管理员。
请求消息示例如下:
```json
{
    "mid": "123456", 
...
以下指令是由平台主动发起请求到设备的指令集，通过该指令集可远程控制设备以及获取设备数据。该指令集指令以2开头。
其中，如果由用户发起的平台指令，消息体中from字段定义为用户ID，设备响应时也应将消息体to字段定义为该用户ID。另外，数据响应格式与3.2章节相同:
```json
{
    "mid": "123456", 
    "to": "用户ID", 
...
平台通过该指令可以控制设备进行日志上传操作，用于远程故障分析。日志文件上传默认采用FTP协议。
请求消息示例如下:
```json
{
    "mid": "123456", 
...
```
如果上传失败，应设置`code`为-1，并给出具体的错误原因。

+ #### <span id="202">设备固件升级通知202</span>

当平台发布了某种产品型号的固件升级版本时，平台会通过该指令广播通知所有该产品型号的设备进行后续升级处理。
通知消息格式如下:
```json
{
    "mid": "123456", 
...
```
设备收到该通知后，如果判断需要升级，应通过[指令103](#103)查询升级版本信息，并根据随机升级时间策略进行升级，不可同一时间同时升级，以免造成服务器升级压力过大。

+ #### <span id="203">团队绑定通知203</span>

平台通过该指令通知设备与用户团队绑定状态的更新。
通知消息格式如下:
```json
{
    "mid": "123456", 
...
```
`status`等于0表示设备和用户团队已解绑，`status`等于1代表设备和团队已绑定。
...
对于某些特殊设备，例如PPT翻页笔，需要用户授权才能够正常使用。通过该指令可实现APP进行远程授权验证并通知设备登录成功。
通知消息格式如下:
```json
{
    "mid": "123456", 
...
```
其中`status`为0表示用户正在进行授权待确认状态（例如用户已扫描二维码），1则表示用户确认授权成功，2表示授权失败。在`status`为0时，不返回`expire`和`token`信息。

注意：**`status`为0的状态并不一定存在，视实际业务场景而定**。
...
所有设备发布到云端应用的消息均通过该指令来完成，平台将负责根据to字段代表的应用服务ID来进行消息转发。
请求消息示例如下:
```json
{
    "mid": "123456", 
...
消息会经过平台验证，然后由平台转发给目标应用。如果平台验证消息失败，会给出如下响应:
```json
{
    "mid": "123456", 
    "from": "system", 
...
所有云端应用服务发布到设备的消息在经过平台验证通过后（具体流程见应用通信协议），都会由平台通过该指令来完成消息转发到设备。其中`from`字段表示发送消息的云端应用ID。
