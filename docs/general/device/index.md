...
|属性|类型|描述|
|----|----|----|
|mid|字符串|消息ID，用来标识一次消息请求上下文，内容由请求方自定义，平台并不对该消息ID进行处理。|
...
设备接入成功后，建议实时调用该指令告知平台更新设备相关的信息，后续设备信息发生变化时，也应通过该指令通知平台进行同步更新。
...
|product_version|字符串|设备产品固件版本号|
|properties|JSON|**可选**，每台设备独特的一些属性参数，设备根据实际情况写入。如果没有，置为空，不上传|
...
设备可通过指令查询平台记录的设备信息。设备本地可以缓存该信息，但应设置合理的过期时间，防止信息不同步。建议设备每天至少查询一次设备状态，但也不可过于频繁。
...
|org_id|字符串|组织ID|
|name|字符串|组织名称|
|status|整型|更新状态。**0**表示设备和用户组织已解绑，**1**代表设备和组织已绑定|
...
设备一旦投入使用，默认都会通过得力APP安装相应的云端应用服务。平台通过该指令可通知设备与云端应用的绑定状态的更新，同样也包括绑定和解绑两种通知。设备不需要对该通知类型进行实时响应。
...
|status|整型|更新状态。**0**表示设备和云端应用已解绑，**1**代表设备和云端应用已绑定|

注意，后续设备与云端应用通过指令[300](#300)通信时需要使用该`app_id`作为`to`字段信息发送应用消息请求。
...
其中，`to`表示目标应用服务ID。如果设备未保存关联应用服务ID在本地，可将其置为空字符串**""**，平台会判断其应用绑定关系进行消息转发（如果设备绑定了多个应用，则会全部转发）。
...
注意，应用服务和设备之间的交互不是实时的，应用服务不会通过300进行消息响应，而是通过下一节的[指令301](#301)。
...
- ### 更新日志
  ...
  ** 1.1 (2017-11-06) **
  ...
  + 重新梳理整个文档，标注消息中的可选信息；
  + 考虑到设备端实现难度，所有id相关信息全部用字符串表示，不再使用长整型；
