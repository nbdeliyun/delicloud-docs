...
任何云端应用服务需要提前在得力云平台进行提前注册，注册时需要提供以下信息:
...
注意: **目前得力云平台并未开放注册，所以云端开发者需要将相关信息提交给平台管理人员，由平台管理人员备案注册 **。

应用注册完成后，平台会提供分配给应用的唯一的`serviceId`和`serviceKey`。后续平台与服务之前的通信将通过该信息进行身份识别和签名。同时，应用也会出现在得力App的应用列表中供用户查看安装。
...
应用服务可以通过以下两种途径被用户安装使用:
    ...
    1.   通过App应用市场或者推荐应用列表，用户搜索然后点击安装；
...
用户登录APP之后，通过App访问云端应用服务的Web页面时，云端应用会授权加载得力云JS SDK的初始化API进行应用授权验证，验证通过后，应用即可通过JS SDK提供的用户API获取当前用户的相关信息，并返回应用界面资源 。
整个授权流程如下:
![](img/image002.png)
...
|请求参数|描述|
|----|----|
|mid|消息ID，用来标识一次消息请求上下文。如果只是消息单向推送，可为空|
|from|发送方ID，根据消息类型ID含义不同|
|action|平台操作指令|
|time|消息发送时间|
|data|具体的消息数据内容，统一为JSON格式|

出于安全考虑，平台发送给服务的回调请求应进行数字签名，服务应对签名进行验证，防止非法请求。签名的计算方式如下:

**`sig = MD5(action + from + serviceId + serviceKey + time)` **

其中签名用的`serviceId`和`serviceKey`是由应用服务注册时生成，sig生成后放到HTTP的请求头部（Request Header）中，与请求参数一同发送给服务。
...
其中，`group_id`表示卸载应用的目标用户组织。

+ #### <span id="402">设备绑定402</span>

通过该指令平台通知应用服务某个个人或企业组织绑定一台新设备。
推送消息格式如下:
```json
{
    "action": 402, 
...
|product_type|产品类型|
|icon|产品图标|
|product_features|产品特征列表，例如支持人脸、指纹考勤等|


+ #### <span id="403">设备解绑403</span>

通过该指令平台通知应用服务某个个人或企业组织解绑一台现有设备。
推送消息格式如下:
```json
{
    "action": 403, 
...
|----|----|
|user_id|平台用户ID|
|evt|具体的用户事件，目前包括的事件有: sign_in:用户登入; sign_off:用户退出; wifi:用户Wifi更新; ble:用户蓝牙更新; gps:用户GPS位置更新|
...
请求内容和格式与WebHook回调接口基本一致，通过`action`来区分不同的消息类型，消息内容封装在`data`里面，平台要求为JSON数据格式。另外，所有HTTP请求均需携带数字签名并附在HTTP请求头部，计算方式如下:

** `sig = MD5(action + from + serviceKey + time)` **

对于发送给设备和用户App的请求，由于是异步消息处理，同步响应结果仅表示平台接收结果，不代表目标设备或用户响应结果。
...
通过该指令应用服务可主动推送消息到设备。设备ID通过`to`字段设置。
请求参数如下:
```json
{
    "from": "发起应用服务ID", 
...
```
由于设备消息属于异步消息，平台会立刻返回推送结果。如果设备不在线，平台会直接返回消息无法送达。例如:
```json
{
    "code": 102, 
    "msg": "设备不在线", 
...
+ #### <span id="504">用户访问授权504</span>

App用户通过应用服务注册地址访问时会携带用户的会话token信息，应用服务应通过此接口与平台通信对用户身份进行验证，并获取用户基本信息。
请求参数如下:
```json
{
    "from": "发起应用服务ID", 
...
后续如果收到平台推送过来的组织更新指令406，应用也应根据最近更新时间进行增量同步操作，保持组织通信录的一致。
请求同步消息格式如下:
```json
{
    "from": "发起应用服务ID", 
...
+ #### <span id="601">用户信息查询601</span>

经过平台预授权，应用服务可通过该指令查询已经保存在平台数据中心的部分用户上报数据。用户特征数据通过特征类型进行区分，应用服务需要指定具体的特征类型来进行查询。
