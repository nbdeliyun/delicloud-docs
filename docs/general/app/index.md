...
平台调用WebHook回调接口统一采用**POST**方式发起请求，请求参数采用JSON格式数据提交，消息格式如下:
```json
{
    "mid": "消息ID", 
    "from": "消息发布者ID", 
...
|from|字符串|消息发布者ID。包括: 设备ID、其它应用服务ID或者平台**system**|
|to|字符串|应用服务ID|
|time|整型|消息发布时间，精确到秒|
...
|org_admin|JSON|组织超级管理员信息，包括：*<br>user_id:管理员用户ID,<br>name:管理员姓名,<br>empno:**可选**，管理员组织工号,<br>avatar:**可选**，管理员头像URL*|

+ #### <span id="401">应用卸载401</span>

通过该指令通知应用服务某个人或企业组织卸载了此应用，该指令与[400](#400)对应。
...
|product|JSON|产品信息。包括: *<br>model:产品型号,<br>name:产品名称, <br>type:产品类型,<br>icon:**可选**，产品图标URL,<br>features:**可选**，产品特征列表，不同产品类型不同，可能为空。包括：<small><br>type: 特征类型(例如支持人脸、指纹), <br>value: **可选**，特征参数（例如人脸算法版本等）</small>*|


+ #### <span id="403">设备解绑403</span>

通过该指令平台通知应用服务某个人或企业组织解绑了一台现有设备。
...
|----|----|----|
|user_id|字符串|用户ID|
|evt|字符串|用户事件，目前包括的事件有: <br>*sign_in:用户登入*,<br>*sign_off:用户退出*, <br>*wifi:用户Wifi更新*, <br>*ble:用户蓝牙更新*, <br>*gps:用户GPS位置更新*|
|evt_data|JSON|**可选**，跟evt相关，不同evt信息不同|
