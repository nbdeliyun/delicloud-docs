...
当用户在云端应用页面选择参与考勤的人员列表后，云端应用需要通过[指令500](../../general/app/#500)通知考勤机，考勤机设备自行判断是否需要更新数据数据，如果需要则设备使用[指令300](../../general/device/#300)通知云端应用需要更新考勤人员数据，云端应用再通过500将考勤人员信息推送给考勤机。
...
在团队管理员通过设备扫描或者主动安装应用时，平台会主动通过WebHook回调通知应用用户安装事件，回调应用指令为[400](../../general/app/#400)。
考勤应用收到安装事件后，应启动初始化，通过数据同步[指令507](../../general/app/#507)开始同步团队数据。如果之前已有团队信息，可通过`update_time`参数进行增量同步。
